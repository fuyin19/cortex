...
- The navigation pair and base Envelope are owned by anti-entropy Core 1.2.1 (ABI `anti-entropy-core.runner/v1`), bound by the installed Cortex launcher to the exact sibling `anti-entropy-core` skill or an explicit absolute `ANTI_ENTROPY_CORE_RUNNER` override. Cortex does not vendor those resources or fall back to a local implementation; it keeps only record/profile/registry/naming responsibilities.
- Distribution `cortex-record-kb`, import `cortex`, version `8.1.1`; profiles Record 1, Tag 2, Layout 5, and exact Registry v1/v2 (`cortex-kb-registry/v2`). Python is exactly 3.11/UCD 14.0.0 for naming. Layout 3 and Layout 4 have no runtime fallback.
...
- Repository skill taxonomy has exactly one discoverable explicit-only skill, `cortex`. Its seven internal roles are documented under `skills/cortex/references/` and use private KB, Notes, and Collaborative Workspace adapters under `skills/cortex/scripts/`; adapter directories never contain `SKILL.md`.
- Build sessions are single-target, explicit, and keyed-monotonic. Existing Tag groups/tags, order, and membership plus Registry id-to-path mappings never contract or reassign; new Tag keys append. Populated Bundles permit complete keyed-monotonic Tag 2 candidates while Layout 5 remains byte-identical. Empty configured and null-sentinel Bundles follow the ordering and first-failure residue rules in `skills/cortex/references/kb-build.md`.
...
- Distribution `cortex-notes`, import `cortex_notes`, version `2.1.0` is separate from `cortex-record-kb`; do not import one from the other.
...
- Notes state is one explicit absolute root containing exact Registry v1/v2 (`cortex-notes-registry/v2`), three Bundles, `.notes.lock`, and independent two-file note units. New roots use v2. Markdown plus `note.json` is the complete source of truth; do not add a database, index, search, vector, server, UI, network, sync, backup, migration, move, restore, or trash surface.
...
- Distribution `cortex-collaborative-workspace`, import `cortex_collaborative_workspace`, version `1.1.3` is isolated from Record KB and Notes. Its closed structured routes are `collaborative_workspace.prepare`, `.status`, and `.validate`, using one explicit absolute `--root`; prepare alone also accepts repeatable `--outdate <relative-source-path>`.
- The outer `collaborative-workspace-envelope/v1` owns fixed navigation, stable UUIDv4 identity, raw human-owned `ref/`, and `agent-workbench/`. The inner `agent-workbench-envelope/v1` owns prepared `ref/`, literal-empty gating `temp/`, and preserved `output/`. Cortex delegates both envelope contracts and every prepared KU validation to the selected Core 1.2.1 runner. Both runtimes preflight its exact ABI/version before business writes, retain it for one operation, and pass that same path to converter subprocesses. Invalid explicit overrides fail without fallback; direct source/library use requires explicit Core configuration.
- Prepare is explicit, nonwaiting, and idempotent: missing-root create, safe ordinary-directory adoption, exact no-op, or stale prepared-ref replacement. It snapshots every ordinary source before fixed routing to file-conversion or markdown-conversion, whose installed runtimes require the matching `file-processing` support skill. It preserves complete basenames, copies valid KUs byte-for-byte, and writes outer `ref/` only to create its fixed `_outdated/` role or fulfill explicit `--outdate`; it never clears temp/output.
- Each conversion route requires its explicit absolute runner environment variable. Its config environment variable is optional when absent; when present, including empty, it must satisfy the same strict absolute ordinary non-link/reparse leaf and ancestor-chain validation and is passed once as `--config`.
- A nonzero conversion provider exit remains `provider_conversion_failed` with its fixed route and exit code. Nonempty stderr is decoded with UTF-8 `backslashreplace`, makes control characters visible except LF/tab, and exposes only the rightmost 4096 characters as `provider_stderr_excerpt` plus `provider_stderr_truncated`; stdout is never returned.
