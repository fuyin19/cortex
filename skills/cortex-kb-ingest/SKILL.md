...
description: Explicit invocation only for adding records or an exact-v1/v2 batch to an already configured Cortex 8.0 Bundle.
---
...
Use this skill only for `record.add` and the skill-local exact-v1/v2 batch wrapper. Never initialize a Bundle, change profiles or Registry 1, edit/show/delete records, infer metadata, repair, migrate, or cut over. The embedded runtime remains the complete closed Cortex 8 CLI; these ownership boundaries are this skill's contract, not runtime route removal.
...
Also set `ANTI_ENTROPY_CORE_RUNNER` to the absolute path to
`anti-entropy-core/scripts/knowledge_unit_runner.py`. Every ingest operation
uses that runner for the base Envelope and has no local fallback.
...
```text
"$CORTEX_PYTHON" -I <ABSOLUTE-SKILL>/scripts/run_cortex.py --json --workspace <bundle> record add --metadata <json> [--source <file>] [--conversion <dir>]
```
