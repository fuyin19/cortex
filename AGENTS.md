...
- `--workspace` is exactly one record-KB root. It contains only `profiles/record-schema.json`, `profiles/tags.json`, `profiles/layout.json`, and direct nonempty partition directories selected through the layout-linked tag group. See `docs/global-knowledge.md` for the authoritative unit and bundle definitions.
...
- `record add` stages one reserved unit sibling in an existing partition, or one complete reserved partition at the bundle root, and publishes with a same-parent no-replace rename. Record edits and profile replacements use a same-directory temporary file and `os.replace`.
