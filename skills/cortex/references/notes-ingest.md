...
Use this role only when the user explicitly invokes `cortex` and the router selects Notes ingest. Generic note, KB, or coding requests are insufficient triggers.
...
```text
"%CORTEX_PYTHON%" -I <ABSOLUTE-CORTEX-SKILL>/scripts/notes/run_notes.py --json --root <absolute-notes-root> [--tools-root <absolute-tools-root>] note add --bundle <bundle> [--partition <partition>] --title <title> --body-file <absolute-markdown> [--timestamp <canonical-+08:00-time>]
```
