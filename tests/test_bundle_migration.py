...
def migration():
    path = Path(__file__).parents[1] / "tools/migrate_legacy_layout3.py"; spec = importlib.util.spec_from_file_location(f"mig_{os.urandom(4).hex()}", path); module = importlib.util.module_from_spec(spec); assert spec and spec.loader; spec.loader.exec_module(module); return module
...
def test_sc024_sc025_sc026_sc027_build_preserves_exact_bytes_and_paths(tmp_path):
    ...
    m.build_bundle(src, output, target, digest, kb_root=kb_root, kb_repo=repo); assert validate_workspace(output).valid
...
def test_sc033_valid_sibling_and_unrelated_entries_are_accepted(tmp_path):
    ...
    assert validate_workspace(output).valid and (kb_root / "notes.txt").read_bytes() == b"ignored"
...
def test_sc034_no_cutover_route_or_function():
    ...
    root = Path(__file__).parents[1]; help_text = (root / "tools/migrate_legacy_layout3.py").read_text("utf-8"); assert "cutover-bundle" not in help_text and "os.replace" not in help_text
    assert [path.name for path in (root / "tools").glob("*migrat*.py")] == ["migrate_legacy_layout3.py"]
