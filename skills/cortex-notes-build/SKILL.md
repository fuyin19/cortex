...
description: Explicit invocation only for initializing or profile-expanding an explicit Cortex Notes 2 Registry and its Bundles.
---

# Cortex Notes build

Use this role only when the user explicitly names `cortex-notes-build`, or explicitly invokes `cortex` and the router selects Notes build. Generic note, KB, or coding requests are insufficient triggers.
