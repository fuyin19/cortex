...
KB 8.1.1 and Collaborative Workspace 1.1.2 require exact anti-entropy Core 1.2.1 via the sibling `anti-entropy-core` skill or explicit `ANTI_ENTROPY_CORE_RUNNER`. Each Core-dependent operation validates ABI/version before writes and fixes that runner for its subprocesses. Notes 2.1.0, help/version, and KB init do not acquire a Core dependency. Conversion providers separately require the matching installed `file-processing` support skill; Cortex does not vendor the conversion runtime.
