...
- Distribution `cortex-collaborative-workspace`, import `cortex_collaborative_workspace`, version `1.0.0` is isolated from Record KB and Notes. Its closed structured routes are `collaborative_workspace.prepare`, `.status`, and `.validate`, using one explicit absolute `--root`.
