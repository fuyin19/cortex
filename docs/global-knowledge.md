...
The two agent skills each carry the same complete Cortex 6.0.0 wheel, manifest, and isolated runner. The formal entry is absolute Python 3.11 with `-I` plus the absolute skill-local runner; there is no installed global command and no PATH, ambient-package, installation, network, sibling-skill, or automatic-update fallback. Runtime packaging changes only delivery and leaves Bundle, Registry, lock, profile, and Result contracts unchanged.
