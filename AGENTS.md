...
- Repository skill taxonomy has one instruction-only non-role router, `cortex`, and exactly six canonical roles: `cortex-kb-ingest`, `cortex-kb-build`, `cortex-kb-manage`, `cortex-notes-ingest`, `cortex-notes-build`, and `cortex-notes-manage`. All seven are explicit-only. The router packages no runtime and selects exactly one role; KB roles embed Cortex 7 and Notes roles embed the separate Cortex Notes 2.0 runtime.
